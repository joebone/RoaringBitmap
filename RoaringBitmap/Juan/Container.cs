...
namespace WebBeds.Indexing
{
    ...
    {
        public const int MaxSize = 4096; // everything greater than this is an ArrayContainer
        ...
        /// <summary>
        /// Will modify X to have the Or'd result of y
        /// </summary>
        /// <param name="x"></param>
        /// <param name="y"></param>
        public static void InlineOrWith(ref Container x, Container y)
        {
            var xArrayContainer = x as ArrayContainer;
            var yArrayContainer = y as ArrayContainer;
            if ((xArrayContainer != null) && (yArrayContainer != null))
            {
                ArrayContainer.InlineOrWithARR(ref x, yArrayContainer);
                return;
            }
            if (xArrayContainer != null)
            {
                ArrayContainer.InlineOrWithBMP(ref x, (BitmapContainer)y);
                return;
            }
            if (yArrayContainer != null)
            {
                BitmapContainer.InlineOrWithARR(ref x, yArrayContainer);
                return;
            }
            BitmapContainer.InlineOrWithBMP((BitmapContainer)x, (BitmapContainer)y);
        }

        /// <summary>
        /// Will modify X to have the And'd result of y
        /// </summary>
        /// <param name="x"></param>
        /// <param name="y"></param>
        public static void InlineAndNotWith(ref Container x, Container y)
        {
            var xArrayContainer = x as ArrayContainer;
            var yArrayContainer = y as ArrayContainer;
            if ((xArrayContainer != null) && (yArrayContainer != null))
            {
                ArrayContainer.InlineAndNotWithARR(ref x, yArrayContainer);
                return;
            }
            if (xArrayContainer != null)
            {
                ArrayContainer.InlineAndNotWithBMP(ref x, (BitmapContainer)y);
                return;
            }
            if (yArrayContainer != null)
            {
                BitmapContainer.InlineAndNotWithARR(ref x, yArrayContainer);
                return;
            }
            BitmapContainer.InlineAndNotWithBMP((BitmapContainer)x, (BitmapContainer)y);
        }
    }
}