...
namespace WebBeds.Indexing
{
    internal class BitmapContainer : Container, IEquatable<BitmapContainer>
    {
        ...
        public static void InlineOrWithARR(ref Container x, ArrayContainer y)
        {
            if (x is BitmapContainer xx)
            {
                xx.m_Cardinality += y.OrArray(xx.m_Bitmap);
            }
        }

        public static void InlineAndNotWithARR(ref Container x, ArrayContainer y)
        {
            if (x is BitmapContainer xx)
            {
                xx.m_Cardinality += y.AndNotArray(xx.m_Bitmap);
            }
        }

        public static void InlineOrWithBMP(BitmapContainer bitmapContainer1, BitmapContainer bitmapContainer2)
        {
            // bitmaps have a fixed size of 1024 ( x 64 )
            var cardinality = OrInternal(bitmapContainer1.m_Bitmap, bitmapContainer2.m_Bitmap);
            bitmapContainer1.m_Cardinality = cardinality;
        }

        public static void InlineAndNotWithBMP(BitmapContainer bitmapContainer1, BitmapContainer bitmapContainer2)
        {
            var cardinality = AndNotInternal(bitmapContainer1.m_Bitmap, bitmapContainer2.m_Bitmap);
            bitmapContainer1.m_Cardinality = cardinality;
        }
    }
}