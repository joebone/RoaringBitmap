...
namespace WebBeds.Indexing
{
    internal class ArrayContainer : Container, IEquatable<ArrayContainer>
    {
        ...
        public static void InlineOrWithARR(ref Container baseContainer, ArrayContainer y)
        {
            if (baseContainer is ArrayContainer x)
            {
                var totalCardinality = x.m_Cardinality + y.m_Cardinality;
                if (totalCardinality > MaxSize)
                {

                    Util.UnionArraysInline(ref x.m_Content, y.m_Content);
                    var calcCardinality = x.m_Content.Length;
                    baseContainer = calcCardinality > MaxSize
                        ? (Container)BitmapContainer.Create(calcCardinality, x.m_Content)
                        : new ArrayContainer(calcCardinality, x.m_Content);
                    return;
                }

                Util.UnionArraysInline(ref x.m_Content, y.m_Content);
                x.m_Cardinality = x.m_Content.Length;
            }
        }

        public static void InlineAndNotWithARR(ref Container baseContainer, ArrayContainer y)
        {
            if (baseContainer is ArrayContainer x)
            {
                Util.AndNotArraysInline(ref x.m_Content, y.m_Content);
                var calcCardinality = x.m_Content.Length;
                baseContainer = calcCardinality > MaxSize
                    ? (Container)BitmapContainer.Create(calcCardinality, x.m_Content)
                    : new ArrayContainer(calcCardinality, x.m_Content);
        ...
        public static void InlineOrWithBMP(ref Container x, BitmapContainer bitmapContainer)
        {
            BitmapContainer output
                = (x is ArrayContainer ar)
                ? BitmapContainer.Create(ar.m_Content)
                : x as BitmapContainer;

            BitmapContainer.InlineOrWithBMP(output, bitmapContainer);
            x = output;
        }
        public static void InlineAndNotWithBMP(ref Container x, BitmapContainer bitmapContainer)
        {
            BitmapContainer output
                = (x is ArrayContainer ar)
                ? BitmapContainer.Create(ar.m_Content)
            ...
            BitmapContainer.InlineAndNotWithBMP(output, bitmapContainer);
            x = output;
        }
    }
}