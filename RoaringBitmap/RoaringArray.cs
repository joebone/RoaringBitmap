    ...
    internal class RoaringArray : IEnumerable<int>, IEquatable<RoaringArray> {
        ...
        public bool Equals(RoaringArray other) {
            ...
            if (Vector.IsHardwareAccelerated && m_Size >= size) {
                Vector<ushort> va = new Vector<ushort>();
                Vector<ushort> vb = new Vector<ushort>();

                //System.Runtime.Intrinsics.Vector256<ushort> mask = new System.Runtime.Intrinsics.Vector256<ushort>();
                //mask = System.Runtime.Intrinsics.Vector256<ushort>
                unsafe {
                    fixed (ushort* src = m_Keys)
                    fixed (ushort* cmp = other.m_Keys) {
                        var i = 0;
                        while (i < m_Size) {
                            var vaa = Avx.LoadAlignedVector256(src);
                            var vab = Avx.LoadAlignedVector256(cmp);

                            var result = Avx2.CompareEqual(vaa, vab);
                            //System.Runtime.Intrinsics.X86.Popcnt

                            if (Avx2.MoveMask(result.AsByte()) == 0x7fffffff) //if (vaa == vab)
                                continue;
